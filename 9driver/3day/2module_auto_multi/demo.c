/*************************************************************************
	...
	> Created Time: Tue 31 Oct 2017 03:46:52 PM CST
 ************************************************************************/
...
static int __init demo_init(void)
{
    ...
    for(i = 0; i < count; i++)
    {
        mdevice = device_create(mclass,NULL, MKDEV(MAJOR(devno),MINOR(devno)+i),NULL,"demo%d",i);
        if(IS_ERR(mdevice))
        {
            ...
            for(j = 0; j < i; j++)
            {
                device_destroy(mclass,MKDEV(MAJOR(devno),MINOR(devno)+j));
            }
            goto err4;
        }
    }
...
static void __exit demo_exit(void)
{
    ...
    for(j = 0; j < count; j++)
    {
        device_destroy(mclass,MKDEV(MAJOR(devno),j));
    }
    class_destroy(mclass);
    cdev_del(pdev);
    kfree(pdev);
